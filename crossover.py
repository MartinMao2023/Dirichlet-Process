...
def crossover(parents):
    ...
    final_index = np.where(np.random.uniform(0, 1, size=(16, 32)) < 0.34286, 
                        permutation, origin_index).reshape(-1)

    large_noise = np.where(np.random.uniform(0, 1, (16, 8)) < 0.3, 1, 0)
    noise = large_noise[:, :, None] * np.random.uniform(-0.25, 0.25, (16, 8, 24)) + np.random.normal(0, 0.1, (16, 8, 24))
