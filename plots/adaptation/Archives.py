...
class Archive_xy(Archive):
    ...
    def build_adaptor(self):
        ...
        for cluster in self.clusters.values():
            ...
            prior_weights.append(cluster["size"]/(total_size + self.alpha))
        ...
        # x_length_scales = []
        # for x_gp in x_gps:
        #     x_length_scales.append(x_gp[2].copy())
        # x_length_scales = np.array(x_length_scales) + 1e-6
        # x_length_scale = np.exp(np.mean(np.log(x_length_scales), axis=0))

        # y_length_scales = []
        # for y_gp in y_gps:
        #     y_length_scales.append(y_gp[2].copy())
        # y_length_scales = np.array(y_length_scales) + 1e-6
        # y_length_scale = np.exp(np.mean(np.log(y_length_scales), axis=0))

        x_gps.append([True, np.zeros((5, 1), dtype=np.float32), np.ones(5), 1, 0])
        y_gps.append([True, np.zeros((5, 1), dtype=np.float32), np.ones(5), 1, 0])
        prior_weights.append(self.alpha/(total_size + self.alpha))
...
class Archive_arc(Archive):
    ...
    def build_adaptor(self):
        ...
        v_gps.append([True, np.zeros((5, 1), dtype=np.float32), np.ones(5), 1, 0])
        w_gps.append([True, np.zeros((5, 1), dtype=np.float32), np.ones(5), 1, 0])
        phi_gps.append([True, np.zeros((5, 1), dtype=np.float32), np.ones(5), 1, 0])
        prior_weights.append(self.alpha/(total_size + self.alpha))
