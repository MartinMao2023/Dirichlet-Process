...
class Adaptor_xy(Adaptor):
    ...
    def read_data(self, data):
        ...
        x_gps[-1][-2] = np.var(x_target) + 0.01
        y_gps[-1][-2] = np.var(y_target) + 0.01
...
class Adaptor_arc(Adaptor):
    ...
    def read_data(self, data):
        ...
        v_gps[-1][-2] = np.var(v_target) + 1e-4
        w_gps[-1][-2] = np.var(w_target) + 1e-4
        phi_gps[-1][-2] = np.var(phi_target) + 1e-4
        ...
        print(posterior_weights[-1])
        # print(self.prior_weights[-1])
