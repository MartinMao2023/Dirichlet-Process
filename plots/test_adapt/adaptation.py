from Adaptor import Adaptor_rte, Combined_adaptor_xy, Combined_adaptor_arc
import numpy as np
import numpy.linalg as lg
import matplotlib.pyplot as plt


coor_x = np.load("coor_x.npy")
coor_y = np.load("coor_y.npy")
coor_v = np.load("coor_v.npy")
coor_w = np.load("coor_w.npy")
coor_phi = np.load("coor_phi.npy")
baselines = np.load("filtered_baseline.npy")

dmg_0_case = np.load("f=0.95_s=0.95_dmg0.npy")
dmg_1_case = np.load("f=0.85_s=0.9_dmg1.npy")
dmg_2_case = np.load("f=0.73_s=0.92_dmg2.npy")
dmg_3_case = np.load("f=0.88_s=0.76_dmg3.npy")
dmg_4_case = np.load("f=0.77_s=0.82_dmg4.npy")

case_dict = {0: dmg_0_case,
             1: dmg_1_case,
             2: dmg_2_case,
             3: dmg_3_case,
             4: dmg_4_case,}

case_index = 3

case = case_dict[case_index]
true_x = case[:, 0]
true_y = case[:, 1]

steps_taken = np.random.choice(len(baselines), size=(100, 26))


# # xy
# xy_adaptation = np.zeros((100, 26), dtype=np.float32)
# for round_index in range(100):
#     adaptor_xy = Combined_adaptor_xy()
#     adaptor_xy.load("combined_xy")
#     errors = np.zeros(26)
#     step_taken = steps_taken[round_index]
#     for i, index in enumerate(step_taken):
#         if i:
#             coor = np.vstack((coor_x[index], coor_y[index]))
#             baseline = baselines[index].reshape(-1, 1)
#             result = case[index].reshape(-1, 1)
#             adaptor_xy.read_data(coor, baseline, result)
        
#         predicted_x, predicted_y = adaptor_xy.predict((coor_x, coor_y), baselines.T)

#         errors[i] = np.mean(np.square(predicted_x[:, 0] - true_x) + np.square(predicted_y[:, 0] - true_y))
#     xy_adaptation[round_index] = errors

# print(xy_adaptation.shape)
# np.save(f"xy_case{case_index}.npy", xy_adaptation)



# # arc
# arc_adaptation = np.zeros((100, 26), dtype=np.float32)
# for round_index in range(100):
#     adaptor_arc = Combined_adaptor_arc()
#     adaptor_arc.load("combined_arc")
#     errors = np.zeros(26)
#     step_taken = steps_taken[round_index]

#     for i, index in enumerate(step_taken):
#         if i:
#             coor = np.vstack((coor_v[index], coor_w[index], coor_phi[index]))
#             baseline = baselines[index].reshape(-1, 1)
#             result = case[index].reshape(-1, 1)
#             adaptor_arc.read_data(coor, baseline, result)
        
#         predicted_x, predicted_y = adaptor_arc.predict((coor_v, coor_w, coor_phi), baselines.T)
#         errors[i] = np.mean(np.square(predicted_x[:, 0] - true_x) + np.square(predicted_y[:, 0] - true_y))
#     arc_adaptation[round_index] = errors

# print(arc_adaptation.shape)
# np.save(f"arc_case{case_index}.npy", arc_adaptation)



# # RTE
# rte_adaptation = np.zeros((100, 26), dtype=np.float32)
# for round_index in range(100):
#     adaptor_rte = Adaptor_rte(v=0.5)
#     errors = np.zeros(26)
#     step_taken = steps_taken[round_index]

#     for i, index in enumerate(step_taken):
        
#         if i:
#             step_data = np.zeros((2, 3))
#             step_data[:, 0] = baselines[index, 0]
#             step_data[:, 1] = baselines[index, 1]
#             step_data[0, 2] = case[index, 0] - baselines[index, 0]
#             step_data[1, 2] = case[index, 1] - baselines[index, 1]
#             adaptor_rte.read_data(step_data)
            
#         predicted_x, predicted_y = adaptor_rte.predict(baselines[:, :2])

#         errors[i] = np.mean(np.square(predicted_x[:, 0] - true_x) + np.square(predicted_y[:, 0] - true_y))
#     rte_adaptation[round_index] = errors

# print(rte_adaptation.shape)
# np.save(f"rte_case{case_index}.npy", rte_adaptation)





# # new xy
# xy_adaptation = np.zeros((100, 26), dtype=np.float32)
# for round_index in range(100):
#     adaptor_xy = Combined_adaptor_xy()
#     adaptor_xy.load("new_adaptor_xy")
#     errors = np.zeros(26)
#     step_taken = steps_taken[round_index]
#     for i, index in enumerate(step_taken):
#         if i:
#             coor = np.vstack((coor_x[index], coor_y[index]))
#             baseline = baselines[index].reshape(-1, 1)
#             result = case[index].reshape(-1, 1)
#             adaptor_xy.read_data(coor, baseline, result)
        
#         predicted_x, predicted_y = adaptor_xy.predict((coor_x, coor_y), baselines.T)

#         errors[i] = np.mean(np.square(predicted_x[:, 0] - true_x) + np.square(predicted_y[:, 0] - true_y))
#     xy_adaptation[round_index] = errors

# print(xy_adaptation.shape)
# np.save(f"new_xy_case{case_index}.npy", xy_adaptation)



# # new arc
# arc_adaptation = np.zeros((100, 26), dtype=np.float32)
# for round_index in range(100):
#     adaptor_arc = Combined_adaptor_arc()
#     adaptor_arc.load("new_adaptor_arc")
#     errors = np.zeros(26)
#     step_taken = steps_taken[round_index]

#     for i, index in enumerate(step_taken):
#         if i:
#             coor = np.vstack((coor_v[index], coor_w[index], coor_phi[index]))
#             baseline = baselines[index].reshape(-1, 1)
#             result = case[index].reshape(-1, 1)
#             adaptor_arc.read_data(coor, baseline, result)
        
#         predicted_x, predicted_y = adaptor_arc.predict((coor_v, coor_w, coor_phi), baselines.T)
#         errors[i] = np.mean(np.square(predicted_x[:, 0] - true_x) + np.square(predicted_y[:, 0] - true_y))
#     arc_adaptation[round_index] = errors

# print(arc_adaptation.shape)
# np.save(f"new_arc_case{case_index}.npy", arc_adaptation)



# possibility changes
weights = []
for round_index in range(100):
    adaptor_xy = Combined_adaptor_xy()
    adaptor_xy.load("combined_xy")
    # adaptor_xy.load("new_adaptor_xy")
    
    posterior_weights = np.zeros((26, len(adaptor_xy.adaptor_xy.posterior_weights) + 1))

    step_taken = steps_taken[round_index]
    for i, index in enumerate(step_taken):
        if i:
            coor = np.vstack((coor_x[index], coor_y[index]))
            baseline = baselines[index].reshape(-1, 1)
            result = case[index].reshape(-1, 1)
            adaptor_xy.read_data(coor, baseline, result)
        
        weight_1, weight_2 = adaptor_xy.weights
        posterior_weights[i, :-1] = weight_1*(adaptor_xy.adaptor_xy.posterior_weights)
        posterior_weights[i, -1] = weight_2
        # predicted_x, predicted_y = adaptor_xy.predict((coor_x, coor_y), baselines.T)

        # errors[i] = np.mean(np.square(predicted_x[:, 0] - true_x) + np.square(predicted_y[:, 0] - true_y))
    
    weights.append(posterior_weights.copy()[None, :, :])

weights = np.concatenate(weights, axis=0)
print(weights.shape)
np.save(f"weights_{case_index}.npy", weights)



