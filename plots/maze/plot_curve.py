...
oracle_mean = np.mean(oracle_projections, axis=0)
...
# plt.fill_between(np.arange(budget + 1), oracle_mean - orcale_std, oracle_mean + orcale_std, color='#2ca02c', alpha=0.2)
plt.plot(np.arange(budget + 1), rte_mean, color="#ff7f0e", label="RTE", linewidth=2)
plt.plot(np.arange(budget + 1), xy_mean, color='#2ca02c', label="DPMM (xy-based)", linewidth=2)
plt.plot(np.arange(budget + 1), arc_mean, color='#9467bd', label="DPMM (arc-based)", linewidth=2, linestyle="-.")
# plt.plot(np.arange(budget + 1), oracle_mean, color='#2ca02c', label="oracle", linewidth=2)
...
plt.title(f"env {case+1}", fontsize=14)
